...
def extract_width_depth_from_filename(filename):
    import re
    pattern = r"depth=(\d+)_width=(\d+)"
...
def main():
    ...
    if args.model_path is not None:
        ...
        state_or_model = ckpt['net']
    
        # 모델 생성: state_dict면 width, depth를 파일명에서 추출
        import collections
        if isinstance(state_or_model, collections.OrderedDict):
            print("Checkpoint contains state_dict. Extracting width and depth from filename...")
            width, depth = extract_width_depth_from_filename(pth_path)
            hidden_dim = width * 32  # 예시: width=2면 hidden_dim=64, width=4면 128
            print(f"Initializing MazeViTUTModel with hidden_dim={hidden_dim}, max_steps={depth}")
            model = MazeViTUTModel(
                in_channels=3,
                hidden_dim=hidden_dim,
                max_steps=depth,
                nhead=4,         # 필요시 파일명에서 추출하거나 고정
                mlp_dim=512,     # 필요시 조정
                img_size=64,     # MazeDataset에 맞게
                patch_size=8,    # MazeDataset에 맞게
                out_channels=2
            )
            model.load_state_dict(state_or_model)
        else:
            print("Checkpoint contains model object.")
            model = state_or_model

        net = model.to(device)
        for name, param in net.named_parameters():
            if "halt" in name:
                print(f"[CHECK] Parameter: {name}, requires_grad={param.requires_grad}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for name, param in net.named_parameters():
            ...
            if param.grad is not None:
                ...
                if torch.max(torch.abs(param.grad)) > 1e3:  # 폭발하는 gradient 감지
